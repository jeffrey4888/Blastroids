...
public class Damage : MonoBehaviour
{
    ...
    private int damageAmountPlayerShield;
    private int damageAmountPlayerBomb;
    ...
    [Tooltip("Whether or not to remove lives on impact")]
    public bool removeLives = false;
    ...
    private void Start()
    {
        if (PlayerPrefs.GetInt("ToggleValue5") == 1 && teamId == 0)
        {
            if (tag == "PlayerProjectile")
            {
                damageAmount = 1;
            }
            if (tag == "PlayerShield" || tag == "PlayerBomb")
            {
                damageAmount = 5;
            }
        }
    ...
    private void DealDamage(GameObject collisionGameObject)
    {
        ...
        if (removeLives && collisionGameObject.tag == "Player")
        {
            collisionGameObject.gameObject.GetComponent<Health>().currentLives = -lifeDamage;
        }
