...
public class Enemy : MonoBehaviour
{
    ...
    [Tooltip("The game manager object. Used for tracking enemy counts.")]
    public GameManager GM;

    [Header("Following Settings")]
    [Tooltip("The transform of the object that this enemy should follow.")]
    public Transform followTarget = null;
    public bool doesFollowTarget = false;
    [Tooltip("The distance at which the enemy begins following the follow target.")]
    public float followRange = 10.0f;

    [Tooltip("Checks if the enemy is summoned. Adds to the static enemy count in the Game Manager.")]
    ...
    /// </summary>
    public enum ShootMode { None, ShootAll, ShootAllDelayed };
    ...
    private void Start()
    {
        ...
        if (shootMode == ShootMode.ShootAllDelayed)
        {
            foreach (ShootingController gun in guns)
            {
                gun.InvokeRepeating("ToggleFireOff", gun.waitTime, gun.waitTime);
            }
        }
    }
    ...
    private void HandleBehaviour()
    {
        ...
        if (followTarget != null && (followTarget.position - transform.position).magnitude < followRange && doesFollowTarget)
        {
            MoveEnemy();
        }
    ...
    private void TryToShoot()
    {
        switch (shootMode)
        {
            ...
            case ShootMode.ShootAll:
                ...
                break;
            case ShootMode.ShootAllDelayed:
                foreach (ShootingController gun in guns)
                {
                    gun.Fire();
                }
